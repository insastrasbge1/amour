...
public class EnteteAfterLogin extends MyHorizontalLayout {
    ...
    private Button vbGestionUtilisateurs;
    ...
    public EnteteAfterLogin(VuePrincipale main) {
        ...
        if (this.main.getSessionInfo().adminConnected()) {
            this.vbGestionUtilisateurs = new Button("Gérer les utilisateurs (admin)");
            this.vbGestionUtilisateurs.addClickListener((event) -> {
                this.main.setMainContent(new UtilisateursBrowser(this.main));
            });
            this.add(this.vbGestionUtilisateurs);
        }
    }
